...
- **Discordアカウント認証**: Supabase Authを使用したOAuth認証
...
2. リダイレクトURIを設定: `https://your-project-ref.supabase.co/auth/v1/callback`
3. Supabaseダッシュボード > Authentication > Providers > Discord にクライアントIDとシークレットを設定

### 環境変数が設定されていない場合

Supabaseが設定されていない場合でも、アプリケーションは動作します：
- Google認証: モック認証が使用されます
- Discord認証: モック認証が使用されます
...
- **OAuth認証**: Google、DiscordなどのOAuth認証が簡単に実装可能
...
2. **Discord認証**: Supabase Auth経由でDiscord OAuth認証
